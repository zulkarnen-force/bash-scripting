...
case "$1" in
    copy)
        ...
        rclone copy "$SOURCE_DIR" "$REMOTE_NAME:$REMOTE_DIR" --progress --dry-run --log-file="$LOG_FILE" --log-level INFO \
          --include-from "$INCLUDE_FILE"
        ;;
    sync)
        echo "[$TIMESTAMP] Starting sync of $SOURCE_DIR to Mega..." >> $LOG_FILE
        rclone sync "$SOURCE_DIR" "$REMOTE_NAME:$REMOTE_DIR" --progress --dry-run --log-file="$LOG_FILE" --log-level INFO \
          --include-from "$INCLUDE_FILE"
        ;;
    remote-sync)
        echo "[$TIMESTAMP] Starting sync from Mega to $SOURCE_DIR..." >> $LOG_FILE
        rclone sync "$REMOTE_NAME:$REMOTE_DIR" "$SOURCE_DIR" --progress --dry-run --log-file="$LOG_FILE" --log-level INFO \
          --include-from "$INCLUDE_FILE"
        ;;
    delete-excluded)
