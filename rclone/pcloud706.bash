...
REMOTE_NAME="pcloud706"      # Replace with your Rclone remote name for PCloud
REMOTE_DIR="Linux/Backup"    # Base destination folder on PCloud
...
INCLUDE_FILE="$HOME/Developments/DevOps/bash-scripting/rclone/pcloud706.txt"        # Path to your include file
...
case "$1" in
    copy)
        echo "[$TIMESTAMP] Starting initial copy of $SOURCE_DIR to PCloud ($REMOTE_DIR_WITH_TIMESTAMP)..." >> $LOG_FILE
        rclone copy "$SOURCE_DIR" "$REMOTE_NAME:$REMOTE_DIR_WITH_TIMESTAMP" --progress --log-file="$LOG_FILE" --log-level INFO \
        --include-from "$INCLUDE_FILE" --exclude-from "$EXCLUDE_FILE"
        ;;
    sync)
        echo "[$TIMESTAMP] Starting sync of $SOURCE_DIR to PCloud ($REMOTE_DIR_WITH_TIMESTAMP)..." >> $LOG_FILE
        rclone sync "$SOURCE_DIR" "$REMOTE_NAME:$REMOTE_DIR_WITH_TIMESTAMP" --progress --log-file="$LOG_FILE" --log-level INFO \
        --include-from "$INCLUDE_FILE"
        ;;
    remote-sync)
        echo "[$TIMESTAMP] Starting sync from PCloud to $SOURCE_DIR..." >> $LOG_FILE
...
if [ $? -eq 0 ]; then
    echo "[$TIMESTAMP] Backup completed successfully to $REMOTE_DIR_WITH_TIMESTAMP." >> $LOG_FILE
else
